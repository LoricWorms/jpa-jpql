...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
				"JOIN a.roles r " +
				"WHERE r.nom = 'Harley Quinn'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
				"JOIN a.roles r " +
				"JOIN r.film f " +
				"WHERE f.annee = 2015", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
				...
				"JOIN r.film f " +
				"JOIN f.pays p " +
				"WHERE p.nom = 'France'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
				...
				"JOIN r.film f " +
				"JOIN f.pays p " +
				"WHERE p.nom = 'France' AND f.annee = 2017", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireRealisateursParActeur() {
		TypedQuery<Realisateur> query = em.createQuery("SELECT DISTINCT r FROM Realisateur r " +
				"JOIN r.films f " +
				"JOIN f.roles ro " +
				"JOIN ro.acteur a " +
				"WHERE a.identite = 'Brad Pitt'", Realisateur.class);
